      ...
      <div className="w-full max-w-4xl px-10">
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-2 rounded-full border-2 border-[#5ECB8A] bg-white/95 backdrop-blur-sm px-3 py-1.5 shadow-sm"
          >
          ...
          <motion.h1 initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.2 }} className="leading-tight">
            <span className="block text-5xl md:text-7xl font-bold text-gray-800 drop-shadow-sm" style={{ fontFamily: "'Comic Sans MS', 'Marker Felt', 'Chalkboard SE', cursive" }}>
              Where Every Pet
            </span>
            <span
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="flex flex-wrap justify-center gap-4 pt-4">
            ...
            {/* Services link depends on auth, without hooks */}
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <SignedIn>
                <Link
                  href={mounted ? '/dashboard' : '/dashboard'}
