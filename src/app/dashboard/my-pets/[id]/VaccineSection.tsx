  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => setShowForm(!showForm)} className="bg-[#f18912] hover:bg-[#458559]">
          <Plus className="h-4 w-4" /> Нэмэх
        </Button>
      </div>
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-4 space-y-3 rounded-xl border border-gray-100 bg-gray-50 p-4">
          <div>
            <Label>Вакцины нэр</Label>
            <Input value={form.vaccine} onChange={(e) => setForm((f) => ({ ...f, vaccine: e.target.value }))} placeholder="Rabies Vaccine" required />
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input type="date" value={form.nextDue} onChange={(e) => setForm((f) => ({ ...f, nextDue: e.target.value }))} required />
            </div>
          </div>
          <div className="flex gap-2">
            <Button type="submit" size="sm" className="bg-[#f18912] hover:bg-[#458559]">
              Хадгалах
            </Button>
        ...
        {pet.vaccines.length === 0 ? (
          ...
          pet.vaccines.map((v, i) => (
            <div key={i} className="flex items-center justify-between rounded-xl border border-gray-100 p-4">
              ...
              <p className="text-sm font-medium text-[#f18912]">{v.nextDue}</p>
            </div>
          ))
        )}
      </div>
